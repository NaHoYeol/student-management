  ...
  if (!assignmentId) {
    return (
      <div className="text-center py-12">
        <p className="text-black">대시보드에서 과제를 선택해 주세요.</p>
    ...
    <div>
      ...
      {!analysis && !loading && !error && (
        <div className="text-center py-20">
          <p className="mb-4 text-black">분석하기 버튼을 눌러 성적 분석을 시작하세요.</p>
          <button
            onClick={runAnalysis}
      ...
      {analysis && (
        <>
          ...
          <div className="mt-8 print:hidden">
            ...
            {studentsLoading ? (
              <p className="text-sm text-black">학생 목록 불러오는 중...</p>
            ) : students.length === 0 ? (
              <p className="text-sm text-black">제출한 학생이 없습니다.</p>
            ) : (
                  ...
                  <div key={s.id} className="rounded-lg bg-white shadow-sm">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-black">{s.studentEmail}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-sm font-bold">
                          {s.score}/{s.totalPoints} ({s.correctRate}%)
                        </span>
                        <span className={`text-black transition-transform ${selectedStudentId === s.studentId ? "rotate-180" : ""}`}>
                          ▼
                        </span>
                      </div>
                    </button>
