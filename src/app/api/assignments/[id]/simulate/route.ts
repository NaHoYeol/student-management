...
## 핵심 원칙: 실제 평가원 기출 데이터 기반 캘리브레이션
⚠️ 난이도 판정은 반드시 **최근 3년간(2022~2024) 평가원 주관 시험(수능, 6월/9월 모의평가)의 실제 정답률 및 선지별 선택률 데이터**를 기준으로 삼으세요.

당신이 문항을 보고 "이건 어려워 보인다/쉬워 보인다"라고 주관적으로 판단하지 마세요. 대신, 해당 문항과 **가장 유사한 유형의 평가원 기출 문항**을 떠올리고, 그 기출의 실제 정답률을 기준으로 난이도를 역산하세요.

캘리브레이션 기준:
- 평가원 기출에서 정답률 85% 이상이었던 유형 → 난이도 1
- 평가원 기출에서 정답률 70~85%였던 유형 → 난이도 2
- 평가원 기출에서 정답률 50~70%였던 유형 → 난이도 3
- 평가원 기출에서 정답률 30~50%였던 유형 → 난이도 4
- 평가원 기출에서 정답률 30% 미만이었던 유형 → 난이도 5

오답 비율도 마찬가지로, 평가원 기출의 선지별 선택률 패턴을 참고하세요:
- 매력적 오답(정답 다음으로 선택률이 높은 선지)은 보통 전체의 15~35%를 차지합니다.
- 나머지 오답은 5~15%씩 분산됩니다.
- 전혀 관련 없는 선지는 3~5% 수준입니다.

이 원칙은 아래 영역별 가이드보다 우선합니다. 영역별 가이드와 평가원 데이터가 충돌하면 평가원 데이터를 따르세요.
...
- **표현법/기법 파악 및 작품 간 비교/공통점·차이점**: ⚠️ **반드시 난이도 1**. 이 유형은 GPT가 가장 과대평가하기 쉬운 유형입니다. 선지에 "역설적 표현", "영탄적 표현", "시각적 이미지", "운율", "부사어", "어순 배열" 등 문학 용어가 아무리 많이 나와도 난이도를 올리지 마세요. 학원생은 이런 용어를 수백 번 반복 훈련했으며, 실제 풀이 과정은 "해당 표현/특징이 작품에 있는가?"를 소거법으로 기계적 확인하는 것입니다. 추론이 아니라 패턴 매칭이므로 난이도 1입니다. (가)와 (나)를 비교하는 문항도 마찬가지로, 각 선지를 작품에 하나씩 대조하면 끝입니다.
  예시1) "(가), (나)에 대한 감상으로 가장 적절한 것은?" + ① 역설적 표현 ② 시어 반복 ③ 지시어 ④ 영탄적 표현 ⑤ 어순 배열 → 난이도 1.
  예시2) "(가), (나)의 공통점으로 적절하지 않은 것은?" + ① 시각적 이미지 ② 자연 관찰→삶 인식 ③ 대조적 시어 ④ 반복 운율 ⑤ 부사어 표현 → 선지마다 작품에 해당 요소가 있는지 확인만 하면 됨. 난이도 1.
...
⚠️ 문학 난이도 판정 시 주의:
- **표현법/기법/공통점·차이점 문항은 절대 난이도 2 이상으로 올리지 마세요.** 선지에 문학 용어(시각적 이미지, 운율, 대조, 반복, 부사어 등)가 나열되어 있으면 복잡해 보이지만, 학생의 실제 풀이는 "각 선지를 작품에 대조 → 있으면 O, 없으면 X"의 단순 확인입니다. 이것은 추론이 아니라 패턴 매칭입니다.
- 현대시/현대소설의 내용 확인, 표현법, 공통점/차이점 문항은 대부분 난이도 1입니다.
