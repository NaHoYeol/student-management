...
주어진 학생 프로필(등급, 특성, 정답/오답 행동 패턴)을 꼼꼼히 읽고, 각 문항에서 이 학생이 실제로 선택할 답을 예측하세요.

핵심 원칙:
1. 정답을 맞히는 것이 목표가 아닙니다. 해당 등급 학생의 인지 수준과 행동 패턴에 맞는 답을 현실적으로 예측하는 것이 목표입니다.
2. 프로필에 기술된 "오답 행동"과 "오답 선택 패턴"을 반드시 반영하세요. 각 등급마다 틀리는 이유와 방식이 다릅니다.
3. 오답을 고를 때 랜덤으로 고르지 마세요. 해당 등급 학생이 실제로 빠지기 쉬운 매력적 오답을 선택하세요.
4. 찍기(같은 번호 3회 이상 연속)는 하지 마세요. 각 문항을 독립적으로 판단하세요.
5. 반드시 제시된 정답률 범위에 맞는 정답 개수를 지켜야 합니다. 이 개수를 초과하거나 미달하면 시뮬레이션이 무의미해집니다.

오답의 질이 중요합니다:
- 상위 등급(1~3): 틀릴 때는 "두 번째로 그럴듯한" 선택지를 골라야 합니다. 말도 안 되는 오답은 절대 안 됩니다.
- 중위 등급(4~6): 부분적으로 맞거나 표면적으로 그럴듯한 오답을 고릅니다. 키워드에 끌려가는 패턴을 반영하세요.
- 하위 등급(7~9): 문제를 제대로 이해하지 못한 상태의 선택입니다. 추측이나 직감에 의한 선택을 반영하세요.
...
전체 ${totalQ}문항 중 정답 ${minCorrect}~${maxCorrect}개, 오답 ${totalQ - maxCorrect}~${totalQ - minCorrect}개.
이 범위를 반드시 지키세요. 위 프로필의 "오답 행동"과 "오답 선택 패턴"에 따라 오답을 선택하세요.
...
위 학생 프로필의 인지 수준과 행동 패턴에 맞게 각 문항의 답을 예측하세요.`,
          },
        ],
        max_tokens: 2048,
        temperature: 0.7,
