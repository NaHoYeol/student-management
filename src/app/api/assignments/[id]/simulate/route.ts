...
### 영어
- **단어/숙어 수준 문항**: 빈칸에 적절한 단어/표현 고르기 (문맥상 명확) → 난이도 1
- **내용 일치/불일치**: 지문 내용과 선지 대조 → 난이도 1
- **주제/요지/제목 파악**: 글 전체를 관통하는 핵심 메시지 파악 → 난이도 1~2
- **빈칸 추론 (연결어/문장)**: 글의 논리적 흐름을 파악하여 빈칸 채우기 → 난이도 2~3
- **빈칸 추론 (추상적 어구)**: 비유적/추상적 표현이 들어가는 빈칸. 글의 주제를 정확히 파악해야 풀림 → 난이도 3~4
- **순서 배열/문장 삽입**: 글의 논리적 구조를 재구성 → 난이도 2~3
- **장문 독해 (복합 지문)**: 긴 지문에서 여러 정보를 종합하여 추론 → 난이도 3~4

### 수학
- **기본 연산/공식 적용**: 공식에 값을 대입하여 계산 → 난이도 1
- **개념 이해 확인**: 그래프 해석, 함수의 성질 파악 → 난이도 1~2
- **조건 해석 + 풀이**: 주어진 조건을 수식으로 변환하여 풀이 → 난이도 2~3
- **다단계 추론**: 여러 개념을 결합하여 풀이 경로를 스스로 설계 → 난이도 3~4
- **킬러 문항 (21, 29, 30번 유형)**: 수열/미적분/기하에서 극한적 사고력 요구. 조건 해석 자체가 난관 → 난이도 4~5

### 사회탐구 (한국사, 사회문화, 생활과 윤리, 윤리와 사상 등)
- **단순 사실 확인**: 역사적 사건, 개념 정의 확인 → 난이도 1
- **개념 비교/구분**: 유사 개념 간 차이점 파악 (예: 기능론 vs 갈등론) → 난이도 1~2
- **자료 해석**: 표, 그래프, 도표를 읽고 해석 → 난이도 2~3
- **사례 적용**: 추상적 개념을 구체적 사례에 적용하여 판단 → 난이도 2~3
- **복합 자료 + 개념 적용**: 여러 자료를 종합하고 개념을 적용하여 추론 → 난이도 3~4

### 과학탐구 (물리, 화학, 생물, 지구과학)
- **단순 개념/용어 확인**: 정의, 법칙 확인 → 난이도 1
- **공식 적용/계산**: 주어진 값으로 공식 적용 → 난이도 1~2
- **실험 해석**: 실험 설계와 결과를 해석 → 난이도 2~3
- **복합 개념 추론**: 여러 단원의 개념을 결합하여 추론 → 난이도 3~4
- **킬러 문항**: 조건이 복잡하고 다단계 논리가 필요한 문항 → 난이도 4~5
