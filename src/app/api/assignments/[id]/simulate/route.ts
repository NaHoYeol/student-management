...
핵심 원칙:
1. 각 문항을 실제로 읽고, 이 학생의 실력 수준에서 풀 수 있는지 스스로 판단하세요. 풀 수 있으면 정답을, 풀 수 없으면 프로필의 오답 패턴에 따라 오답을 선택하세요.
2. 정답/오답 개수를 인위적으로 맞추지 마세요. 문항 난이도와 학생 프로필의 상호작용에 따라 자연스럽게 결정되어야 합니다.
3. 프로필에 기술된 "정답 행동"을 보고 이 학생이 해결할 수 있는 문항인지 판단하고, "오답 행동"과 "오답 선택 패턴"을 보고 틀릴 때 어떤 선지를 고를지 결정하세요.
4. 오답을 고를 때 랜덤으로 고르지 마세요. 해당 등급 학생이 실제로 빠지기 쉬운 매력적 오답을 선택하세요.
5. 찍기(같은 번호 3회 이상 연속)는 하지 마세요. 각 문항을 독립적으로 판단하세요.
...
각 문항을 실제로 읽고, 위 학생의 실력과 행동 패턴에 비추어 이 학생이 선택할 답을 예측하세요. 문항 난이도에 따라 맞힐 수 있는 건 정답을, 이 학생 수준에서 풀 수 없는 건 프로필의 오답 패턴대로 오답을 선택하세요.`,
          },
        ],
        max_tokens: 2048,
        temperature: 0.7,
