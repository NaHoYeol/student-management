...
- **표현법/서술 기법 파악**: 비유, 역설, 반어, 서술 시점 등 기법 확인 → 난이도 1~2. 단, 선지에서 기법을 잘못 명명한 함정(예: "역설"이 아닌 "반어")이 있으면 난이도 2~3.
- **작품 간 비교/공통점·차이점**: 두 작품(현대시+고전시, 소설+수필 등)의 공통된 정서나 차이를 파악 → 난이도 2~3.
- **<보기> 기반 감상**: <보기>가 제시하는 관점(시대적 배경, 작가 의도, 비평 이론 등)을 렌즈로 삼아 작품을 재해석. 학생이 <보기>를 정확히 이해하지 못하면 연쇄 오답 → 난이도 2~3.
- **선지의 절반-정답 함정**: "화자의 상실감이 자연물에 투영되어 있다" vs "화자의 상실감이 자연물을 통해 극복되고 있다"처럼 핵심 서술어 하나로 정오가 갈림. 상위권도 함정에 빠지는 유형 → 난이도 3~4.
- **고전 시가/산문 해석 + 감상**: 고어 해석이 필요한 작품에서 <보기> 적용까지 요구되면 2중 부하 → 난이도 3~4.

⚠️ 문학 난이도 판정 시 주의:
- 현대시/현대소설의 내용 확인 문항은 대부분 난이도 1~2입니다. 과대평가하지 마세요.
- 단, 고전 작품이 포함된 문항은 현대 작품 동일 유형보다 +1 가산을 고려하세요.
- 선지 수준의 미세한 차이를 구분해야 하는 문항은 작품 자체가 쉬워도 난이도 3 이상입니다.
