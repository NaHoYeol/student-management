...
function NotificationBell({ isSuperAdmin }: { isSuperAdmin?: boolean }) {
  ...
  const fetchCount = useCallback(() => {
    fetch("/api/admin/notifications")
      .then((r) => r.json())
      .then((data) => {
        const total = (data.pendingParents || 0) + (data.pendingInstructors || 0);
        setCount(total);
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <Link
      href={isSuperAdmin ? "/superadmin/instructors" : "/admin/parents"}
...
export function Nav() {
  ...
  const superAdminLinks = [
    { href: "/superadmin/instructors", label: "강사 관리" },
    ...adminLinks,
  ];

  const links = isSuperAdminRole
    ? superAdminLinks
    : isAdminRole
    ? adminLinks
    : isParent
  ...
  const homeHref = isSuperAdminRole
  ...
  const roleBadge = isSuperAdminRole ? "관리자" : isAdminRole ? "강사" : isParent ? "학부모" : "학생";
    ...
    <nav className="border-b bg-white px-4 py-3 sm:px-6">
      <div className="mx-auto flex max-w-6xl items-center justify-between">
        ...
        <div className="hidden items-center gap-2 sm:flex">
          <ThemeToggle />
          {isAdminRole && <NotificationBell isSuperAdmin={isSuperAdminRole} />}
